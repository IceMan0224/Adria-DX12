...
namespace adria
{
	...
	class VulkanRayTracingShaderBindings;

	class VulkanCommandList final : public GfxCommandList
	{
	public:
		VulkanCommandList(GfxDevice* gfx, GfxCommandListType type, Char const* name = "");
		virtual ~VulkanCommandList() override;
		ADRIA_NONCOPYABLE(VulkanCommandList)
		ADRIA_DEFAULT_MOVABLE(VulkanCommandList)

		virtual GfxDevice*       GetDevice() override               { return (GfxDevice*)gfx; }
		virtual void*            GetNative() const override         { return (void*)cmd_buffer; }
		...
		virtual void TextureBarrier(GfxTexture const& texture, GfxResourceState flags_before, GfxResourceState flags_after, Uint32 subresource = static_cast<Uint32>(-1)) override;
	...
	private:
		...
		std::vector<std::pair<VkSemaphore, Uint64>> pending_waits;
		std::vector<std::pair<VkSemaphore, Uint64>> pending_signals;

		std::vector<VkImageMemoryBarrier2>  pending_image_barriers;
		std::vector<VkBufferMemoryBarrier2> pending_buffer_barriers;
		std::vector<VkMemoryBarrier2>       pending_global_barriers;

		std::unique_ptr<VulkanRayTracingShaderBindings> current_rt_bindings;

		alignas(8) Uint8 push_constants[VK_PUSH_CONSTANT_SIZE]{};
		alignas(4) Uint8 root_constant_shadow[VK_ROOT_CONSTANT_SIZE]{};
	};
}
